...
Configure the protected `crates-io` and `crates-io-rc` GitHub environments with required reviewers. Store the restricted crates.io token as `CARGO_REGISTRY_TOKEN` in both environments. Only the marked-push job enters `crates-io-rc`; pull requests and unmarked pushes run a separate job with no Cargo credential.
